...
Many times, you will see this expression written as $\lim\limits_{x \to c^{-}} f(x) = L$ or $\displaystyle\lim_{x \to c^{+}} f(x) = L$. 
...
.pull-right[
Let's consider even more complicated function, $y = x-x^2$, plotted to the left. What is its ''slope'', or $\frac{\Delta f(x)}{\Delta x}$?
...
If both the numerator and the denominator are $0$, $\infty$, $-\infty$, we cannot evaluate the limit.

--

However, **L'Hôpital's Rule** says:
